...
| Image Compression | 上传前图片压缩模式，可选 off / auto        | 否  | `off`                                       |
...
`Image Compression` 默认值为 `off`，会完全保持旧行为，不压缩、不缩放、不转格式。

选择 `auto` 时，插件会在图片体积较大时尝试压缩，适合整屏截图、网页截图、较大的 PNG 截图。内部策略固定为：

- 图片超过 700KB 时尝试压缩
- 保留原始分辨率，不缩放图片
- 输出 JPEG，quality 为 0.85
- 只有体积减少至少 15% 才使用压缩图，否则回退原始 PNG
- 压缩失败时静默回退原图，不影响 OCR 请求

推荐设置：

- 追求最高识别质量：`off`
- 日常截图 OCR、希望减少上传体积：`auto`

如果识别小字、代码、表格、公式时质量下降，建议改回 `off`。
